...
def MPS(fitness, mating_pool_size):
    ...
    selected_to_mate = []
    total_fitness = sum([f[0] for f in fitness])
    pointers_prep = []
    pointers = []
    for i in [f[0] for f in fitness]:
        pointers_prep.append(i / total_fitness)
    for i in range(len(pointers_prep)):
        pointers.append(sum(pointers_prep[:i + 1]))
...
for i in range(MAX_GENERATIONS):
    ...
    selected_indices = MPS(scores, POPULATION_SIZE)
